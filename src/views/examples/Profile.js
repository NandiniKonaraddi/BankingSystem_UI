...
const Profile = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const formData = {firstName,lastName,address,city,country,postalCode,aboutMe};
    try {
      formData.username = username.username
      formData.email = username.email
      Rootservice.profile(formData).then((res) => {
  ...
  const selfIntroduction = `My name is ${firstName} ${lastName}. I currently live in ${city}, ${country} (Postal Code: ${postalCode}). ${aboutMe}`;
  ...
  useEffect(() => {
    const getTokenFromLocalStorage = async () => {
      ...
      if (token) {
        ...
        Rootservice.getprofile(value).then((res) => {
            ...
            <Card className="bg-secondary shadow">
              <CardHeader className="bg-white border-0">
                <Row className="align-items-center">
                  <Col xs="8">
                    <h3 className="mb-0">My Personal Details</h3>
                  </Col>
